...
public class New_AddDeviceActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
        ...
        filter.addAction(New_AddDeviceActivity.class.getName());
	...
	*/
	...
	private View.OnClickListener btnScanClickListener = new View.OnClickListener() {
		...
		public void onClick(View v) {
    		...
    		mintent = mintent.setClass(New_AddDeviceActivity.this,qr_codeActivity.class); 
    		New_AddDeviceActivity.this.startActivityForResult(mintent,REQUEST_CODE_GETUID_BY_SCAN_BARCODE );
		}
	};
	...
	private View.OnClickListener btnSearchOnClickListener = new View.OnClickListener() {
		...
		public void onClick(View v) {
			...
			AlertDialog.Builder builder = new AlertDialog.Builder(new ContextThemeWrapper(New_AddDeviceActivity.this, R.style.HoloAlertDialog));
	...
	private View.OnClickListener btnOKOnClickListener = new View.OnClickListener() {
		...
		public void onClick(View v) {
			...
			if (dev_nickname.length() == 0) {
				MainActivity.showAlert(New_AddDeviceActivity.this, getText(R.string.tips_warning), getText(R.string.tips_camera_name), getText(R.string.ok));
				return;
			}

			if (dev_uid.length() == 0) {
				MainActivity.showAlert(New_AddDeviceActivity.this, getText(R.string.tips_warning), getText(R.string.tips_dev_uid), getText(R.string.ok));
				return;
			}

			if (dev_uid.length() != 20) {
				MainActivity.showAlert(New_AddDeviceActivity.this, getText(R.string.tips_warning), getText(R.string.tips_dev_uid_character), getText(R.string.ok));
				return;
			}
			
			if (!dev_uid.matches("[a-zA-Z0-9]+")) 
			{
				MainActivity.showAlert(New_AddDeviceActivity.this, getText(R.string.tips_warning), getText(R.string.tips_dev_uid_special_characters), getText(R.string.ok));
				return;
			}
			
			if (view_pwd.length() == 0) {
				MainActivity.showAlert(New_AddDeviceActivity.this, getText(R.string.tips_warning), getText(R.string.tips_dev_security_code), getText(R.string.ok));
				return;
			}
			
			thread = new ThreadTPNS(New_AddDeviceActivity.this,dev_uid,0);
			thread.start();
...
//		    HttpGetTool.execute(com.tutk.P2PCam264.DatabaseManager.s_GCM_PHP_URL+"?cmd=reg_mapping&token="+com.tutk.P2PCam264.DatabaseManager.s_GCM_token+"&appid="+com.tutk.P2PCam264.DatabaseManager.s_Package_name+"&uid="+dev_uid+"&imei="+tm.getDeviceId());
			...
			if (duplicated) {
				MainActivity.showAlert(New_AddDeviceActivity.this, getText(R.string.tips_warning), getText(R.string.tips_add_camera_duplicated), getText(R.string.ok));
				return;
			}
				
				
			/* add value to data base */
			DatabaseManager manager = new DatabaseManager(New_AddDeviceActivity.this);
			long db_id = manager.addDevice(dev_nickname, dev_uid, "", "", "admin", view_pwd, 3, channel);

			Toast.makeText(New_AddDeviceActivity.this, getText(R.string.tips_add_camera_ok).toString(), Toast.LENGTH_SHORT).show();

			/* return value to main activity */
			  EventBus.getDefault().post(  
                      new Add_Device_Event(db_id,dev_nickname,dev_uid,"admin",view_pwd,0));  
			...
			New_AddDeviceActivity.this.setResult(RESULT_OK, Intent);
			New_AddDeviceActivity.this.finish();
		}
	};
