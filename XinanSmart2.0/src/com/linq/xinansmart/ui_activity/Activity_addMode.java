...
public class Activity_addMode extends Activity {
	...
	protected void onActivityResult(int requestCode, int resultCode,
			final Intent data) {
		if (resultCode == 11112222 && requestCode == 11122) {
				...
				if (isChecked == true)// 互联网模式才有定时开启功能
				{
					...
					et_startTime.setOnClickListener(new OnClickListener() {
						...
						public void onClick(View v) {
							 ...
							 */
							LinearLayout layout = (LinearLayout) Activity_addMode.this
							...
							ad = new AlertDialog.Builder(Activity_addMode.this)
	...
	class ChioceOnclicked implements OnClickListener {
		...
		@Override
		public void onClick(View v) {
			Intent intent = new Intent(Activity_addMode.this,
...
//	}
	...
	class SaveClicked implements OnClickListener {
		...
		public void onClick(View v) {
			...
			if (TextUtils.isEmpty(et_modeName.getText())) {
				Toast.makeText(Activity_addMode.this, "场景名称不能为空",
						Toast.LENGTH_SHORT).show();

			}
			if (et_modeName.getText().length() > 20
					|| et_modeCode.getText().length() > 20) {
				Toast.makeText(Activity_addMode.this, "请输入20字以内内容",
						Toast.LENGTH_SHORT).show();
			...
			if (!TextUtils.isEmpty(et_modeName.getText())
				...
				ProgressDialog progressDialog = new ProgressDialog(
						Activity_addMode.this);
				progressDialog.setTitle("正在保存");
				progressDialog.show();
				//performAnimate((Button) v);
				Runnable run = new Runnable() {
					...
					public void run() {
						if (timingStr.equals("TRUE")) {
							...
							mode = ModeManager.getInstance(concenter)
									.AddAppProfile(
											et_modeCode.getText().toString(),
											et_modeName.getText().toString(),
											false,
											"\\/Date(1422158721471+0800)\\/",
											"\\/Date(1422158721471+0800)\\/",
											"\\/Date(1422158721471+0800)\\/",
						...
						ModeManager manager = ModeManager
								.getInstance(concenter);
						for (m = 0; m < lstViewEquip.size(); m++) {
							equi = EquipmentManager.getInstance().getEqById(
									lstViewEquip.get(m).getViewId());
							String curvalue = lstViewEquip.get(m)
									.getViewValue();
	...
	public class MyAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			convertView = View.inflate(Activity_addMode.this,
			...
			if (type == 1 || type == 2 || type == 3 || type == 4 || type == 5
				...
				if(vequp.getViewValue()!=null){
					if (vequp.getViewValue().equals("TRUE")) {
						svalue = "开";
					}
				}
...

			{
				...
				int a = Integer.parseInt(vequp.getViewValue());
				if (a > 0) {
					seek.setProgress(60);
				} else {
					seek.setProgress(0);
				...
				button.setVisibility(View.GONE);
				seek.setVisibility(View.VISIBLE);
				seek.setProgress(Integer.parseInt(vequp.getViewValue()));
				seek.setOnSeekBarChangeListener(new OnSeekBarChangeListener() {
					@Override
					public void onStopTrackingTouch(SeekBar seekBar) {
						ViewEquipment veq = (ViewEquipment) seekBar.getTag();
